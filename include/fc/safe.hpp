...
template <typename Stream, typename T> Stream& operator<<(Stream& stream, const safe<T>& obj)
{
    stream << obj.value;
    return stream;
}

template <typename Stream, typename T> Stream& operator>>(Stream& stream, safe<T>& obj)
{
    stream >> obj.value;
    return stream;
}
